...
class SecondDerivativeFourthOrderCenter(SpatialDerivative):
    ...
    def derivative_xi(self, primes: jax.Array, dxi: jax.Array, i: int) -> jax.Array:
        s1_ = self.s_[i]
        deriv_xi = (1.0 / 12.0 / dxi / dxi) * (- primes[s1_[0]] + 16.0 * primes[s1_[1]] - 30.0 * primes[s1_[2]] + 16.0 * primes[s1_[3]] - primes[s1_[4]])
        return deriv_xi

    def derivative_xi_xj(self, primes: jax.Array, dxi: jax.Array, dxj: jax.Array, i: int, j: int) -> jax.Array:
