...
class MaterialManager():
    ...
    def get_thermal_conductivity(self, T: jax.Array):
    ...
    def get_dynamic_viscosity(self, T: jax.Array):
        if self.levelset_type == "FLUID-FLUID":
    ...
    def get_psi(self, p: jax.Array, rho: jax.Array) -> jax.Array:
        if self.levelset_type == "FLUID-FLUID":
            psi = []
            for i, fluid in enumerate(self.materials):
    ...
    def get_grueneisen(self, rho: jax.Array) -> jax.Array:
        if self.levelset_type == "FLUID-FLUID":
            grueneisen = []
            for i, fluid in enumerate(self.materials):
